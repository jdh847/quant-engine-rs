...
- `dashboard_cover.html` plus `dashboard_cover_github.html` / `dashboard_cover_x.html` / `dashboard_cover_xiaohongshu.html` for repo/social cover-style export use
...
- strategy comparison now supports market + plugin + command filters, so you can isolate US / A / JP experiments, zoom in on a single alpha/plugin, and separate `run` / `demo` / `research-report`
